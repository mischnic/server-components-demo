...
type Context = {|
  parentNode: BundlerBundleGraphTraversable,
...
function shouldSkipServer(
  /*:: this: MutableBundleGraph,*/ d /*: Dependency */
) /* : boolean*/ {
  if (this.isDependencySkipped(d)) return true;
  let resolution = this.getDependencyResolution(d);
  if (isServerAsset(resolution)) return true;
  return false;
}
...
module.exports = (new Bundler({
  ...
  bundle({bundleGraph}) {
    ...
    let rootClientAsset /* : ?Asset */;
    let rootClientAssetContext /* : ?Context */;
    let serverBundleGroup /* : ?BundleGroup */;

    // Step 1: create bundles for each of the explicit code split points.
    bundleGraph.traverse({
      enter: (node, context, actions) /* : Context */ => {
        ...
        if (resolution && !rootClientAsset && isServerAsset(resolution)) {
          let parent = nullthrows(context?.parentNode);
          invariant(parent.type === 'asset');
          rootClientAsset = parent.value;
          let ctx = {
            ...context,
            bundleGroup: context?.bundleGroup,
            bundleByType: context?.bundleByType,
        ...
        if (context && context.parentNode) {
          ...
          if (
            resolution &&
            isNonClientAsset(context.parentNode.value) &&
            ...
            // $FlowFixMe
            context = {
              ...rootClientAssetContext,
              parentNode: {value: rootClientAsset, type: 'asset'},
            };
          }
        }
    ...
    for (let [bundle, rootAssets] of bundleRoots) {
      for (let asset of rootAssets) {
        bundleGraph.addEntryToBundle(
          asset,
          bundle,
          shouldSkipServer.bind(bundleGraph)
        );
      }
    }
  },
